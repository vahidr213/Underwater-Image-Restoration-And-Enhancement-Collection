function imrestored = main2(doDegradation,inpath,paramvector)
% pre-processing hue with homomorphic function
% subtract two optimum constant from A1,H1,V1,D1
...
if (paramvector(1) < 0) || (paramvector(1) >= 0.15) || (paramvector(2)<0) || (paramvector(2)>=0.3) 
    ...
    I = im2double(I_in);
    I = rgb2hsv(I);
    I(:,:,1) = homomorphic1(I(:,:,1), [paramvector(3),paramvector(4)] );
    namewavelet = 'sym3';
    N = 1;
    n=1;
...
function Is = strechimage1(data,s)
    ...
    function g=homomorphic1(f,v)
        J=double(f);
        f_high = 2.5;
        f_low = 0.5;
        
        %gauss_low_filter = fspecial('gaussian', [7 7], 1.414);
        gauss_low_filter = fspecial('gaussian', floor([v(1) v(1)]), v(2));
